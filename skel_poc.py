...
# V110.3.20.6 · MINIMAL DIRECT PIPELINE
...
def _segment_relative_lower_seed_112(torch,model,pose,rotvec,trans,scale,target_df,reference_target_df,betas,zero_trans,reference_pose,axis_map,calibration=None):
    """V110.3.20.6 · Retargeting segmentario absoluto respecto al frame semilla.
    ...
    """
    out={'strategy':'V110.3.20.6 segment-relative hip + geometric knee','accepted':False,'chains':{}}
    try:
...
# V110.3.20.6 · FULL-BODY SEGMENT-DRIVEN RETARGETING
...
def _smooth_fullbody_q_sequence_206(torch,model,sequence,betas,zero_trans,axis_map,scale):
    """V110.3.20.6 · refinamiento temporal segmentario conservador.

    Mantiene intacto el retargeting segmentario de pelvis/cadera/rodilla/tronco y
    sólo refina temporalmente los DOF ya observados. La prioridad de esta versión
    son q7/q14 (tobillos), con mayor amortiguación del izquierdo, que en V110.3.20.6
    mostró el mayor salto frame-a-frame. El brazo izquierdo conserva el refinado
    estable de V110.3.20.6.

    Importante: no se crea excursión nueva ni se copia información contralateral.
    El filtro opera únicamente sobre la q(t) ya obtenida del driver segmentario.
    """
    ok_idx=[i for i,f in enumerate(sequence) if f.get('status')=='ok' and f.get('pose') is not None]
    qids=[3,4,5,6,7,10,11,12,13,14,26,27,28,29,30,31,32,36,37,38,39,40,41,42]
    if len(ok_idx)<7:
        return {'applied':False,'reason':'menos de 7 frames'}

    P=np.asarray([sequence[i]['pose'] for i in ok_idx],dtype=np.float64)
    raw=P[:,qids].copy(); sm=raw.copy()
    k5=np.asarray([1,2,3,2,1],dtype=np.float64)/9.0
    k7=np.asarray([1,2,3,4,3,2,1],dtype=np.float64)/16.0
    k9=np.asarray([1,2,3,4,5,4,3,2,1],dtype=np.float64)/25.0

    left_arm={36,37,38,39,40,41,42}
    diag={}

    # Configuración específica. q14 recibe el mayor refinamiento porque era el
    # tobillo con mayor brusquedad en V110.3.20.6. q7 también se estabiliza, pero
    # de forma más conservadora para preservar la excursión distal ya conseguida.
    cfg={
        7:  {'kernel':k7,'pad':3,'weight':0.72,'rom_floor':0.94,'cap_deg':4.5,'label':'ankle_right_temporal_refinement'},
        14: {'kernel':k9,'pad':4,'weight':0.84,'rom_floor':0.93,'cap_deg':4.2,'label':'ankle_left_temporal_refinement'},
    }

    for j,qi in enumerate(qids):
        x=raw[:,j]
        if qi in cfg:
            c=cfg[qi]; pad=np.pad(x,(c['pad'],c['pad']),mode='reflect')
            f=np.convolve(pad,c['kernel'],mode='valid'); a=float(c['weight'])
            floor=float(c['rom_floor']); hard=np.deg2rad(float(c['cap_deg']))
            method=c['label']
        elif qi in left_arm:
            pad=np.pad(x,(3,3),mode='reflect'); f=np.convolve(pad,k7,mode='valid'); a=.78
            floor=.90; hard=np.deg2rad(3.6); method='left_arm_v204_preserved'
        else:
            pad=np.pad(x,(2,2),mode='reflect'); f=np.convolve(pad,k5,mode='valid'); a=.68
            floor=.92; hard=np.deg2rad(5.0); method='general_segment_conservative'
        ...
        # Frame inicial exactamente anclado a la solución segmentaria original.
        y=y+(x[0]-y[0])

        # Limitador robusto bidireccional. Para q7/q14 el límite es absoluto y no
        # se relaja por un pico presente en la señal cruda; así se elimina justo la
        # brusquedad que motivó esta versión.
        d=np.diff(y); med=float(np.median(np.abs(d))) if len(d) else 0.0
        cap=hard if qi in cfg else max(hard,3.0*med)
        for _ in range(4 if qi in cfg else 2):
            for n in range(1,len(y)):
                dd=y[n]-y[n-1]
                if abs(dd)>cap:
                    y[n]=y[n-1]+np.sign(dd)*cap
        ...
        sm[:,j]=y
        raw_step=float(np.degrees(np.max(np.abs(np.diff(x))))) if len(x)>1 else 0.0
        new_step=float(np.degrees(np.max(np.abs(np.diff(y))))) if len(y)>1 else 0.0
        raw_acc=float(np.degrees(np.max(np.abs(np.diff(x,n=2))))) if len(x)>2 else 0.0
        new_acc=float(np.degrees(np.max(np.abs(np.diff(y,n=2))))) if len(y)>2 else 0.0
        diag[_SKEL_Q_NAMES_130[qi]]={
            'raw_range_deg':float(np.degrees(rr)),
            'refined_range_deg':float(np.degrees(np.ptp(y))),
            'rom_retained_ratio':float(np.ptp(y)/rr) if rr>1e-8 else None,
            'raw_max_step_deg':raw_step,'refined_max_step_deg':new_step,
            'raw_max_accel_proxy_deg':raw_acc,'refined_max_accel_proxy_deg':new_acc,
    ...
    for r,i in enumerate(ok_idx):
        ...
        rv=torch.tensor(fr.get('rotvec',[0,0,0]),dtype=torch.float32); tr=torch.tensor(fr.get('trans',[0,0,0]),dtype=torch.float32)
        with torch.no_grad():
            W,_,_=_direct_world_130(torch,model,pt,betas,zero_trans,rv,scale,tr,axis_map=axis_map)
        fr['pose']=pt.cpu().numpy()[0].astype(float).tolist(); fr['joints']=W.cpu().numpy().astype(float).tolist()
        fr['segment_q']={_SKEL_Q_NAMES_130[q]:float(pt[0,q].cpu()) for q in qids}

    return {
        'applied':True,
        'method':'V110.3.20.6 segment ankle temporal refinement; q7 7-point FIR/4.5deg cap; q14 9-point FIR/4.2deg cap; left-arm V204 preserved',
        'qids':qids,'per_q':diag,
        'raw_ranges':np.ptp(raw,axis=0).astype(float).tolist(),
        'refined_ranges':np.ptp(sm,axis=0).astype(float).tolist(),
        'ankle_targets':{
            'q7':{'rom_floor':0.94,'cap_deg_per_frame':4.5,'filter_weight':0.72},
            'q14':{'rom_floor':0.93,'cap_deg_per_frame':4.2,'filter_weight':0.84},
        },
    }
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    smooth=_smooth_fullbody_q_sequence_206(torch,model,sequence,betas,zero_trans,axis_map,scale)
    result={'version':'110.3.20.6','retargeting_mode':'segment_ankle_temporal_refinement','scale':float(scale.cpu()),
            ...
            'segment_refinement_206':_json_safe_134(smooth)}
...
# V110.3.20.6 · SKEL LOWER-LIMB MOTOR ISOLATION TEST
...
def _build_skel_skin_sequence(torch, model, seq, progress_cb=None):
    ...
    return {
        "version":str((seq or {}).get("version","110.3.20.6")),
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        if Jviz is not None and pelvis_idx is not None and len(Jviz)==len(Vviz):
            ...
            # V110.3.20.6 · orientación VISUAL fija desde frame 1:
        ...
        const layout={{title:'V110.3.20.6 · SKEL ANIMATED ANATOMICAL MESH · anatomical-axis centered playback',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        ...
        st.caption("V110.3.20.6 · visor centrado en pelvis y orientado a ejes anatómicos sólo para presentación. El NPZ científico conserva las coordenadas originales. Usa ‘⬇ Vídeo SKEL’ para grabar y descargar la marcha anatómica desde el visor.")
    except Exception as exc:
...
def _lower_limb_vector_dls_196(torch,model,pose,rotvec,trans,scale,target_df,rows,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.20.6 · propagación q(t) de miembros inferiores basada en VECTORES DE SEGMENTO.
    ...
    audit={'accepted':False,'strategy':'V110.3.20.6 relative-segment weighted-XYZ DLS','axis_weights':w.astype(float).tolist(),'chains':{}}
    any_commit=False
...
def _lower_limb_delta_tracking_199_fast(torch,model,pose,rotvec,trans,scale,target_df,prev_target_df,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.20.6 · solver inferior quirúrgico y rápido.
    ...
    try:
        ...
        chains=[('R',[3,4,5,6],('RHip','RKnee','RAnkle'),('femur_r','tibia_r','talus_r')),
                ('L',[10,11,12,13],('LHip','LKnee','LAnkle'),('femur_l','tibia_l','talus_l'))]
        audit={'accepted':False,'strategy':'V110.3.20.6 fast frame-delta DLS','axis_weights':w.tolist(),'chains':{}}
        with torch.no_grad():
            leg_ids=[3,4,5,6,7,10,11,12,13,14]
        ...
        return pose,{'accepted':False,'strategy':'V110.3.20.6 fast frame-delta DLS','error':f'{type(exc).__name__}: {exc}','non_blocking':True}
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    frames=list((motion or {}).get("frames") or [])
    # V110.3.20.6 MINIMAL DIRECT: sin persistencia de secuencia/malla/NPZ.
    if not frames:
    ...
    st.caption("V110.3.20.6 · flujo directo mínimo: sin migración de NPZ y sin recuperación/persistencia de secuencias o mallas previas. Cada ejecución parte de los 75 frames V104/V107 activos y genera q(t) → skin_verts(t) → malla en memoria.")
    ...
    st.markdown("**Modelo SKEL privado · V110.3.20.6 · Lower-Limb q(t) Propagation Fix**")
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    export={
                        "version":"110.3.20.6","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                    ...
                    if isinstance(_mmesh,dict) and isinstance(_mmesh.get('vertices'),np.ndarray):
                        ...
                        st.download_button("⬇️ Descargar Motor Test SKEL (.NPZ)",_mesh_sequence_npz_bytes(_mmesh),"V110_3_20_6_SKEL_lower_limb_motor_test.npz","application/octet-stream",use_container_width=True)

                    st.markdown("### V110.3.20.6 · Propagación temporal 1→75")
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        # V110.3.20.6 · sólo memoria de sesión. Si q(t) existe y la malla no,
                        ...
                        st.markdown("### V110.3.20.6 · SKEL ANIMATED ANATOMICAL MESH")
                        ...
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            st.markdown('### V110.3.20.6 · Validación sincronizada Side-by-Side')
                        ...
                        if frame_gate_ok:
                            st.info("✅ El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.3.20.6 generará automáticamente skin_verts(t) y la malla corporal animada en la misma ejecución.")
                        else:
